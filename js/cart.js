...
function process() {
    ...
    if (storageList.length == 0) {
        ...
        for (var i = 0, len = storageList.length; i < len; i++) {
            ...
            str +=
                '<figcaption> <a href="#" class="info text-dark text-justify"><b>' +
                storageList[i].name +
                '</b></a>';
            str +=
                ' <p class=" moreInfo small ">' + storageList[i].info + '</p>';
            str += ' </figcaption>';
            str += '</figure>';
            str += ' </td >';
            str += '<td>';
            ...
            str +=
                '<div class="priceWrap">' +
                internationalNumberFormat.format(totalEach).toString() +
                'đ</div>';
            str += '</td>';
            str +=
                "<td><button onclick=\"lsdel('items','" +
                i +
                '\');" class="xoa btn btn-outline-danger p-1">Remove</button></td>';
            str += '</tr > ';
        }
    }
